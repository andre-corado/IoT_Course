...
def insertar_humedad(conn, valor, ubicacion, estado, timestamp=None):
    """Inserta un registro de humedad en la base de datos"""
    try:
        cursor = conn.cursor()
        ...
        if timestamp:
            ...
            query = """
                INSERT INTO humedad (timestamp, valor, ubicacion, estado)
                VALUES (%s, %s, %s, %s)
            """
            ...
            query = """
                INSERT INTO humedad (valor, ubicacion, estado)
                VALUES (%s, %s, %s)
            """
            cursor.execute(query, (valor, ubicacion, estado))
        
        conn.commit()
        print(f"Registro de humedad insertado: {valor}%, {ubicacion}, {estado}")
        cursor.close()
        return True
    except mysql.connector.Error as err:
...
def on_message(client, userdata, msg):
    try:
        ...
        # Guardar en MySQL
        if conn:
            insertar_humedad(conn, valor, ubicacion, estado, timestamp)
        else:
            print("Error: No hay conexión a MySQL disponible")
            
        # Mostrar información recibida
        print(f"\nHumedad recibida: {valor}%")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Iniciando suscriptor para datos de humedad...")
